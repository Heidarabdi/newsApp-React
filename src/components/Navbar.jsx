...
function Navbar() {
  ...
  return (
    <header className="navbar w-full h-15 bg-[#fff] mb-5 shadow-md p-2 z-50">
      {/* Desktop view */}
      <nav className="flex items-center justify-between text-[#2A2A2A] px-3 md:px-8">
        ...
        <div className={`hidden md:block font-[MaryToddW00-Regular] text-2xl space-x-4`}>
          {navBar.map((item, index) => (
            <Link
              className={`${hover === index ? 'underline underline-offset-4' : ''}`}
              to={item.to}
              key={index}
            >
              {item.name}
      ...
      {/* Mobile view */}
      <div className={`flex flex-col ${open ? 'absolute' : 'hidden'} z-20 bg-white right-0 top-0 md:w-[25rem] w-full drop-shadow-2xl shadow-black h-screen text-center`}>
